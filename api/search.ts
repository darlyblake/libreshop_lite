...
// Simple serverless handler for /api/search
...
// Simple serverless handler for /api/search
// Intended for Vercel/Netlify serverless functions or a lightweight Node server.
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const page = Number(req.query.page || req.body.page || 1);
    const perPage = Number(req.query.perPage || req.body.perPage || 10);
    if (!q || q.trim().length === 0) return res.status(200).json({ data: [], total: 0 });
