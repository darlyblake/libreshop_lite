  ...
  const chartDataWithMA = useMemo(() => {
    ...
    return data
      ...
      .map((point, index, arr) => {
        ...
        return { ...point, movingAverage: Math.round(ma) };
      });
  }, [data]);
  ...
  if (!font) {
    return (
      <View style={{ minHeight: height, justifyContent: 'center', alignItems: 'center' }}>
        <ActivityIndicator size="small" color={appliedColor} />
      </View>
    );
  }

  return (
    <View style={{ minHeight: height, backgroundColor: cardColor }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <TouchableOpacity
          ...
          style={{
            flexDirection: 'row',
            alignItems: 'center',
            backgroundColor: showMA ? `${appliedColor}15` : '#f3f4f6',
            paddingHorizontal: 12,
            paddingVertical: 6,
            borderRadius: 16,
          }}
        >
          <Ionicons name={showMA ? 'trending-up' : 'trending-up-outline'} size={16} color={showMA ? appliedColor : '#9ca3af'} style={{ marginRight: 6 }} />
          <Text style={{ fontSize: 12, fontWeight: '600', color: showMA ? appliedColor : '#9ca3af' }}>Tendance</Text>
        </TouchableOpacity>
      </View>

      {loading ? (
        <View style={{ height: 240, justifyContent: 'center', alignItems: 'center' }}>
          <ActivityIndicator size="large" color={appliedColor} />
        </View>
        ...
        <View style={{ height: 240 }}>
          <CartesianChart data={chartDataWithMA} xKey="date" yKeys={showMA ? ['revenue', 'movingAverage'] : ['revenue']} padding={{ left: 45, right: 25, top: 20, bottom: 45 }} axisOptions={axisOptions}>
            {({ points }) => (
              <>
                <Area points={points.revenue} y0={0} color={`${appliedColor}12`} />
                <Line points={points.revenue} color={appliedColor} strokeWidth={3.8} animate={{ type: 'timing', duration: 300 }} />
                {showMA && points.movingAverage && (
                  <Line points={points.movingAverage} color="#f59e0b" strokeWidth={2.2} animate={{ type: 'timing', duration: 300 }}>
          ...
          <Animated.View style={[{ position: 'absolute', top: 25, backgroundColor: '#fff', padding: 12, borderRadius: 12, borderWidth: 1, borderColor: '#e5e7eb', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 10, elevation: 6, zIndex: 100, pointerEvents: 'none', minWidth: 165 }, tooltipStyle]}>
            {tooltipData && (
              <>
                <Text style={{ fontSize: 13, color: '#64748b' }}>{new Date(tooltipData.date).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' })}</Text>
                <Text style={{ fontSize: 18, fontWeight: '800', color: appliedColor, marginTop: 4 }}>{tooltipData.revenue.toLocaleString('fr-FR')} FCFA</Text>
              </>
            )}
          </Animated.View>
        </View>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center', flexWrap: 'wrap', gap: 20, marginTop: 12, paddingHorizontal: 10 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <View style={{ width: 14, height: 14, backgroundColor: `${appliedColor}20`, borderRadius: 4, borderWidth: 2, borderColor: appliedColor }} />
