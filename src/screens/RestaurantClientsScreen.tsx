...
const RESTAURANT_COLORS = ['#FF6B35', '#FF8C42'] as const;
...
const LOYALTY_TIERS: Record<string, { label: string; color: string; emoji: string }> = {
  ...
  Platinum: { label: 'Platine', color: '#E5E4E2', emoji: '💎' },
};
...
const getAvatarColor = (name: string) => {
  const colors = ['#FF6B35', '#FF8C42', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6'];
  const idx = name.charCodeAt(0) % colors.length;
  return colors[idx];
};

export const RestaurantClientsScreen: React.FC = () => {
  ...
  const loadClients = useCallback(async () => {
    ...
    try {
      ...
      // getByStore returns orders; group by customer
      ...
      (orders.items || []).forEach((order: any) => {
        ...
        if (order.created_at > clientMap[key].lastOrder) {
          clientMap[key].lastOrder = order.created_at;
        }
      });
      const clientList = Object.values(clientMap).sort((a: any, b: any) => b.visits - a.visits);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      >
        <Text style={{ color: '#fff', fontSize: 22, fontWeight: '800' }}>Clients fidèles 👥</Text>
        ...
        {FILTER_TABS.map(tab => (
          <TouchableOpacity
            ...
            style={{
              ...
              paddingVertical: 8,
              borderRadius: 20,
              backgroundColor: activeFilter === tab.id ? '#FF6B35' : COLORS.card,
            }}
          >
        ...
        >
          {filteredClients.length === 0 ? (
            <View style={{ alignItems: 'center', paddingVertical: 60 }}>
              ...
              <Text style={{ color: COLORS.textMuted, fontSize: 16, marginTop: 12 }}>Aucun client trouvé</Text>
            </View>
            ...
            return (
              ...
              >
                ...
                <View style={{ alignItems: 'flex-end', gap: 4 }}>
                  <View style={{
                    backgroundColor: tierConfig.color + '33',
                    paddingHorizontal: 8,
                    paddingVertical: 4,
                    borderRadius: 8,
                  }}>
